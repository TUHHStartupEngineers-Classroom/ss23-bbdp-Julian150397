...
function(user_input) {
  user_input %>%
    pluck(.default = "Error") %>% str_split_i(pattern = fixed(","), 1)
}
get_stock_data <-
function(stock_symbol, 
  ...
  stock_symbol %>% quantmod::getSymbols( 
    ...
    mutate(currency = case_when(
      str_detect(names(.) %>% dplyr::last(), "DE") ~ "dollar",
      TRUE                                   ~ "euro")) %>% 
...
function(stock_data) {
  ...
  g <- stock_data %>% 
    ...
    pivot_longer(cols = c("adjusted", "mavg_short", "mavg_long"), 
      ...
      scale_y_continuous(labels = stock_data %>% pull(currency) %>% xts::first() %>% currency_format()) +
